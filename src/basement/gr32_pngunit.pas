...
uses
  Classes, SysUtils, Graphics32, GR32_PortableNetworkGraphic;
...
type
  ...
  private
    ...
    procedure AssignPropertiesFromBitmap32(Bitmap32: TBitmap32);
  protected
  ...
  public
    ...
    procedure DrawToBitmap32(Bitmap32: TBitmap32); virtual;
...
begin
 if Bitmap is TBitmap32
  then Result := TBitmap32(Bitmap).PixelPtr[0,Y]
  else Result := nil;
end;
...
procedure TPortableNetworkGraphic32.DrawToBitmap32(Bitmap32: TBitmap32);
var
...
procedure TPortableNetworkGraphic32.AssignPropertiesFromBitmap32(Bitmap32: TBitmap32);
var
  Index         : Integer;
  IsAlpha       : Boolean;
  IsGrayScale   : Boolean;
...
begin
 if Source is TBitmap32 then
  with TBitmap32(Source) do
   begin
    // Assign
    AssignPropertiesFromBitmap32(TBitmap32(Source));
   ...
   with DataStream do
    try
     with EncoderClass.Create(DataStream, FImageHeader, FGammaChunk, FPaletteChunk) do
      try
       if Assigned(FProgressEvent)
        then EncodeFromScanline(TBitmap32(Source), GR32ScanlineProgress)
        else EncodeFromScanline(TBitmap32(Source), GR32Scanline);
      finally
       Free;
      end;
...
begin
 if Dest is TBitmap32 then
  begin
   TBitmap32(Dest).Width := ImageHeader.Width;
   TBitmap32(Dest).Height := ImageHeader.Height;
   DrawToBitmap32(TBitmap32(Dest));
  end
 else inherited;
end;
