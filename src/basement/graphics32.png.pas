...
begin
  if Bitmap is TBitmap32 then
    Result := TBitmap32(Bitmap).PixelPtr[0, Y]
  else
    Result := nil;
end;
        ...
        begin
          if IsPalette then
            ...
            begin
			  ...
			  with TColor32Entry(Color^) do
			  begin
	              TempPalette[PalIndex].R := R;
	              TempPalette[PalIndex].G := G;
	              TempPalette[PalIndex].B := B;
			  end;
    ...
    with DataStream do
      try
        with EncoderClass.Create(DataStream, FImageHeader, FGammaChunk, FPaletteChunk) do
          try
            if Assigned(FProgressEvent) then
              EncodeFromScanline(TBitmap32(Source), GR32ScanlineProgress)
            else
              EncodeFromScanline(TBitmap32(Source), GR32Scanline);
          finally
            Free;
          end;
...
begin
  ...
  begin
    TBitmap32(Dest).SetSize(ImageHeader.Width, ImageHeader.Height);
